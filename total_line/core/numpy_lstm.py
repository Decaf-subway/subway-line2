...
class NumpyLSTMModel:
    """Inference-only NumPy runner for the saved 1-8 line LSTM models."""

    def __init__(self, model_path):
        self.model_path = model_path
        self.weights = self._load_weights(model_path)
        self.uses_boarding_pred = self.weights["dense0_w"].shape[0] == 53
    ...
    def _load_weights(cls, model_path):
        ...
        with h5py.File(io.BytesIO(weights_bytes), "r") as h5:
            return {
                ...
                "line_embedding": cls._read_dataset(h5, "layers/embedding_1/vars/0"),
                ...
                "dense2_w": cls._read_dataset(h5, "layers/dense_2/vars/0"),
                "dense2_b": cls._read_dataset(h5, "layers/dense_2/vars/1"),
            }
    ...
    def __call__(self, inputs, training=False):
        ...
        line_idx = np.asarray(inputs[2]).astype(int).reshape(-1)
        ...
        line_emb = w["line_embedding"][line_idx]
        parts = [x, station_emb, line_emb]

        if self.uses_boarding_pred:
            boarding_pred = np.asarray(inputs[3], dtype=np.float32).reshape(seq.shape[0], 1)
            parts.append(boarding_pred)

        x = np.concatenate(parts, axis=1)
        x = _relu(x @ w["dense0_w"] + w["dense0_b"])
        x = _relu(x @ w["dense1_w"] + w["dense1_b"])
        return x @ w["dense2_w"] + w["dense2_b"]
